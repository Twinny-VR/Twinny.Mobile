...
namespace Twinny.Multiplatform.Navigation
{
    /// <summary>
    /// Moves a NavMeshAgent to tapped positions and optionally reports interactable hits.
    /// </summary>
    [RequireComponent(typeof(NavMeshAgent))]
    public class FpsNavigation : MonoBehaviour, IPlatformInputCallbacks, IPlatformCallbacks
    {
        ...
        public void OnSelect(SelectionData selection)
        {
            ...
            if (_agent == null)
            {
                Debug.LogWarning("[FpsNavigation] NavMeshAgent is null.");
                return;
            }
            ...
            Debug.Log(
                $"[FpsNavigation] OnSelect {hit.collider.name} at {hit.point} " +
            ...
            {
                Debug.Log($"[FpsNavigation] Interactable hit {hit.transform.name}");
        ...
        private bool TryMoveTo(Vector3 worldPos)
        {
            ...
            {
                Debug.Log($"[FpsNavigation] NavMesh.SamplePosition failed at {worldPos}");
                return false;
            }

            Debug.Log(
                $"[FpsNavigation] Moving to {navHit.position} " +
        ...
        private void TryTeleportToPendingLandmark()
        {
            ...
            {
                Debug.LogWarning($"[FpsNavigation] Pending landmark '{landmarkGuid}' was not found in LandmarkHub.");
                return;
            }
