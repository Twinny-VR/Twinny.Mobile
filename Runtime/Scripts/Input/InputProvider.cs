...
namespace Twinny.Multiplatform.Input
{
    /// <summary>
    ...
    /// </summary>
    public class InputProvider : TSingleton<InputProvider>
    {
        private const string SettingsResourceName = "InputSettings";
...
#endif
            if (FindAnyObjectByType<InputProvider>() != null) return;
            var providerObject = new GameObject("InputProvider");
            providerObject.AddComponent<InputProvider>();
        ...
        private void HandleSingleTouch()
        {
            ...
            switch (t.phase)
            {
                case TouchPhase.Began:
                    ...
                    {
                        CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnHapticTouch());
                        InputEvents.HapticTouch();
                    }
                    break;
                ...
                case TouchPhase.Moved:
                    if (Vector2.Distance(t.position, _touchStartPos) > _settings.DragThreshold)
                    {
                        ...
                        InputEvents.Drag(t.deltaPosition, t.position);
                    }
                    break;
        ...
        private void HandleMouseInput()
        {
            ...
            else if (isMouseHeld)
            {
                ...
                if (_isDragging)
                {
                    ...
                    InputEvents.Drag(delta, currentMousePos);
                }
            ...
            else if (isMouseUp)
            {
                ...
                if (!_isDragging)
                {
                    ...
                    InputEvents.Tap(currentMousePos);
                }
        ...
        private void HandleTwoFingers()
        {
            ...
            if (t0.phase == TouchPhase.Moved || t1.phase == TouchPhase.Moved)
            {
                ...
                if (_isZooming && Mathf.Abs(pinchDelta) > 0.0001f)
                {
                    ...
                    InputEvents.PinchZoom(pinchDelta);
                }
                ...
                else if (_isPanning)
                {
                    ...
                    if (centerDelta.sqrMagnitude > 1.0f)
                    {
                        ...
                        CallbackHub.CallAction<IPlatformInputCallbacks>(
                            cb => cb.OnTwoFingerSwipe(panDelta, centerPos)
                        );
                        InputEvents.Drag(panDelta, centerPos);
                    }
                }
                ...
                t0.phase == TouchPhase.Canceled || t1.phase == TouchPhase.Canceled)
            {
                ...
                InputEvents.Drag(Vector2.zero, center);
            }
            ...
            if (t0.phase == TouchPhase.Ended && t1.phase == TouchPhase.Ended)
            {
                if (Time.time - _lastTwoFingerPressTime < 0.3f)
                {
                    ...
                    // Fire both events
                    CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnTwoFingerTap(center));
                    InputEvents.Tap(center);
                }
            }
        ...
        private void HandleThreeFingers()
        {
                ...
                t2.phase == TouchPhase.Moved)
            {
                ...
                InputEvents.Drag(avgDelta.normalized, centerPos);
                ...
                if (Mathf.Abs(pinchDelta) > 0.01f)
                {
                    ...
                    InputEvents.ThreeFingerPinch(pinchDelta);
        ...
        private void HandleFourFingers()
        {
            ...
            {
                // Four-finger tap
                CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnFourFingerTap());
                InputEvents.FourFingerTap();
            }
                ...
                t3.phase == TouchPhase.Moved)
            {
                ...
                InputEvents.Tap(avgDelta.normalized);
            }
        }
        ...
        private void DetectDevicePhysics()
        {
                ...
                Time.time - _lastShakeTime > 1.0f)
            {
                ...
                CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnShake());
                InputEvents.Shake();
            }
                ...
                UnityInput.deviceOrientation != DeviceOrientation.Unknown)
            {
                ...
                InputEvents.DeviceRotated(_lastOrientation);
            }
        }
        ...
        private void DetectPickupAndPutdown()
        {
            ...
            if (!_isDevicePickedUp && accel.magnitude > _settings.PickupAccelerationThreshold)
            {
                ...
                CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnPickUp());
                InputEvents.PickUp();
            }
            ...
            if (_isDevicePickedUp && accel.magnitude < 0.2f)
            {
                ...
                {
                    _isDevicePickedUp = false;
                    CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnPutDown());
                    InputEvents.PutDown();
                }
            }
            else
            ...
            if (UnityInput.touchCount > 0)
            {
                foreach (Touch touch in UnityInput.touches)
                {
                    if (touch.phase == TouchPhase.Began)
                    {
                        ...
                        if (edge.HasValue)
                        {
                            CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnEdgeSwipe(edge.Value));
                            InputEvents.EdgeSwipe(edge.Value);
                        }
                    }
                }
            }
        ...
        private void ProcessTap(Vector2 position)
        {
            ...
            {
                Debug.Log($"[InputProvider] Tap hit {hit.collider.name} at {hit.point}");
                SelectionData selection = new SelectionData(hit);
                router.Select(selection);
                CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnSelect(selection));
            }
            else
            {
                Debug.Log("[InputProvider] Tap hit nothing.");
                router.Cancel();
            }
        }
        ...
        private void ProcessTap(Touch t)
        {
            ...
            if (UnityInput.touchPressureSupported && t.maximumPossiblePressure > 0)
            {
                ...
                {
                    CallbackHub.CallAction<IPlatformInputCallbacks>(cb => cb.OnForceTouch(pressure));
                    InputEvents.ForceTouch(pressure);
                }
            }
        }
        ...
        private void CheckAccessibilityActions()
        {
            ...
            if (UnityInput.touchCount == 1 && UnityInput.GetTouch(0).tapCount == 2)
            {
                ...
                if (_isScreenReaderActive)
                {
                    ...
                    InputEvents.AccessibilityAction("DoubleTap");
                }
            }
            ...
            if (UnityInput.touchCount == 1)
            {
                ...
                if (t.phase == TouchPhase.Began && t.position.y > Screen.height - 100)
                {
                    ...
                    InputEvents.NotificationAction(true);
                }
            }
        }
        ...
        private void EnsureSettingsLoaded()
        {
            ...
            _settings = Resources.Load<InputSettings>(SettingsResourceName);
            if (_settings != null) return;
            ...
            if (!_warnedMissingSettings)
            {
                ...
                Debug.LogWarning(
                    $"[InputProvider] Missing settings asset. " +
                    $"Expected Resources/{SettingsResourceName}.asset. " +
                    "Using in-memory defaults."
                );
            }

            _settings = ScriptableObject.CreateInstance<InputSettings>();
        }
        ...
        private InputRouter TryGetRouter()
        {
            ...
            if (router == null && !_warnedMissingRouter)
            {
                ...
                Debug.LogWarning("[InputProvider] InputRouter.Instance is null. Input routing disabled.");
            }
            return router;
        }
        ...
        public void TriggerAccessibilityAction(string actionName)
        {
            ...
            InputEvents.AccessibilityAction(actionName);
        }
        ...
        public void TriggerScreenReaderGesture(string gestureType)
        {
            ...
            InputEvents.ScreenReaderGesture(gestureType);
        }
