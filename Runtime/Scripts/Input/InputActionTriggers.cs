...
namespace Twinny.Multiplatform.Input
{
    ...
    public class InputActionTriggers : MonoBehaviour, IPlatformInputCallbacks
    {
        [SerializeField] private List<InputAction> _inputActions = new List<InputAction>();

        private void OnEnable() => CallbackHub.RegisterCallback<IPlatformInputCallbacks>(this);
        private void OnDisable() => CallbackHub.UnregisterCallback<IPlatformInputCallbacks>(this);

        #region Trigger Methods
        private void TriggerAction(InputAction.ActionType type)
        {
            Debug.LogWarning($"[InputActionTriggers] {type} triggered.");
        ...
        #region IPlatformInputCallbacks Implementation
        // Core Callbacks
