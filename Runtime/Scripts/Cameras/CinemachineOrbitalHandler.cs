...
namespace Twinny.Multiplatform.Cameras
{
    ...
    public class CinemachineOrbitalHandler : MonoBehaviour, IPlatformInputCallbacks, IPlatformCallbacks
    {
        ...
        private void EnsureReferences()
        {
            ...
            if (_demoMode == null)
                _demoMode = GetComponent<CinemachineDemoMode>();
        }

        private void ApplyMode(bool isActive, bool applyPendingLandmark = true)
        {
            if(isActive) CallbackHub.CallAction<IPlatformUICallbacks>(callback => callback.OnMaxWallHeightRequested(AlphaClipper.MinMaxWallHeight.y));
        ...
        private void RefreshTargetOverrides(bool forceRefresh = false)
        {
            ...
            if (_isModeActive)
            {
                CallbackHub.CallAction<IPlatformUICallbacks>(
        ...
        private void WarnMissingRadiusOnce()
        {
            ...
            Debug.LogWarning(
                "[CinemachineOrbitalHandler] Could not find radius field on CinemachineOrbitalFollow."
            );
        }
        ...
        {
            if (!PlatformManager.TryGetPendingLandmarkGuid(out string landmarkGuid))
                return;

            if (!LandmarkHub.TryGetByLandmarkGuid(landmarkGuid, out Landmark landmark) || landmark == null)
            {
                Debug.LogWarning($"[CinemachineOrbitalHandler] Pending landmark '{landmarkGuid}' was not found in LandmarkHub.");
                return;
            }
